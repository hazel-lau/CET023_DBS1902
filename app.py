...
def index():
    if request.method == "POST": 
        ...
        s1 = "Predicted DBS share price based on Linear Regression model is: " + str(pred[0][0])
        model = joblib.load("DBSDT")
        pred = model.predict([[float(rates)]])
        s2 = "Predicted DBS share price based on Decision Tree model is: " + str(pred[0])
        model = joblib.load("DBSNN")
        pred = model.predict([[float(rates)]])
        s3 = "Predicted DBS share price based on Neural Network model is: " + str(pred[0])
